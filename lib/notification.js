...
export async function notifyAppointmentEvent(appointment, eventType, extraLines = []) {
  ...
  if (eventType === "reminder" && waEnabled) {
    const waResult = await sendWhatsappNotification(appt, message, eventType);
    return { whatsapp: waResult };
  } else {
    const smsResult = await sendSmsNotification(appt, message, eventType);
